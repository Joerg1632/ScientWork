...
std::vector<double> calculateAnalyticalM() {
    ...
    for (int t = 0; t < num_steps; ++t) {
        ...
        double coeff2 = (0 * nu - initial_N * lambda) / (lambda + nu);
...
void saveToCSV(const std::vector<double>& mean,
    ...
    const std::string unique_path = getUniqueFilename(
        "SimOfMachFailure/results/resultWeibullGroup.csv");
    ...
    outfile << "Time;Weibull mean;Weibull mean+Sqrt(Var);"
...
void readInput(const std::string& filename) {
    ...
    while (std::getline(infile, line)) {
        ...
        switch (line_count) {
            ...
            case 1: iss >> lambda_w;       break;
            case 2: iss >> delta_shape;    break;
            case 3: iss >> nu;             break;
            case 4: iss >> nu_w;           break;
            case 5: iss >> initial_N;      break;
            case 6: iss >> delta_t;        break;
            case 7: iss >> T;              break;
            case 8: iss >> num_experiments;break;
        }
        if (iss.fail()) {
    ...
    if (line_count < 9) {
        std::cerr << "Ожидалось 9 параметров, считано " << line_count << std::endl;
        exit(EXIT_FAILURE);
    }
    infile.close();
}
